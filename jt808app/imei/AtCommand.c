...
JNIEXPORT jint JNICALL Java_AtCommand_Ql_1SendAT
  (JNIEnv *env, jobject thisObj, jstring atcmdstr, jstring finalRspstr, jcharArray strResponse, jlong timeout_ms) {
   ...
   char* array = (*env)->GetCharArrayElements(env, strResponse, NULL);
   uint32_t arraysize = (*env)->GetArrayLength(env, strResponse);

   smd_fd = open(QUEC_AT_PORT, O_RDWR | O_NONBLOCK | O_NOCTTY);
   printf("< open(\"%s\")=%d >\n", QUEC_AT_PORT, smd_fd);
   
   jint ret = Ql_SendAT(atCmd, finalRsp, array, timeout_ms);
   // start to send AT...
   close(smd_fd);
