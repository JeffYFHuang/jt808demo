...
//		_atc = atc;
    ...
    private boolean parseNMEA(String nmea) {
        if (Logger.BUILD_DEBUG) {
            Logger.log("GpsNmeaParser.parseNMEA( \"" + nmea + "\" );", true);
        }
        ...
        if (aNMEA[0].compareTo("GPRMC") == 0) {
            try {
                ...
                if (aNMEA[7].length() > 0) {
                    spd = Double.parseDouble(aNMEA[7]);
        ...
        if (Logger.BUILD_DEBUG) {
            Logger.log("aNMEA[9].length(): " + aNMEA[9].length(), true);
        }
...
/*                comm = (CommConnection) Connector.open("comm:com" + portNumber + ";baudrate=" + portSpeed + ";blocking=on;autocts=off;autorts=off");
                ...
                commIs = new FileInputStream("/dev/smd7");
                commOs = new FileOutputStream("/dev/null");
            }
        } catch (Exception ex) {
