...
public class Packet {
  ...
  */
  ...
  public Packet(short id,
    ...
    switch (cipher) {
      ...
      default:
        ...
        Logger.log(TAG + "  Packet: Unknown cipher mode, set to none.");
    }
    ...
    if (phone != null && phone.length == 6) {
      ...
      Logger.log(TAG + "  Packet: Illegal phone number, set to empty.");
    }
    ...
    if (payload != null) {
      ...
      Logger.log(TAG + "  Packet: Payload not specified, set to empty");
    }
  ...
  public Packet(byte[] raw) {
    ...
    if (checksum != ArrayUtils.xorCheck(Arrays.copyOfRange(main, 0, main.length - 1))) { //does it incloud the byte of checksum? Jeff
      throw new IllegalArgumentException("XOR check failed."); //mark for pass? Jeff
    }
