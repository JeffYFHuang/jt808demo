package com.cva.jt808.conn;

import com.cva.javacint.logging.Logger;
import com.cva.jt808.filter.MessageFilter;
...
//      }
  ...
  private class ConnectBG implements Runnable {
    ...
    void start(){
      Logger.log(TAG + "  start connect thread");
  ...
  private void connectComplete(Socket socket) throws IOException{
    synchronized (conLock){
      ...
      Logger.log(TAG + "  connect SUCCESS !!!");
  ...
  private class CloseBG implements Runnable {
    ...
    void start(){
      Logger.log(TAG + "  start close thread");
      mDisConnThread.start();
    }

    @Override
    public void run() {
      Logger.log(TAG + "  closing");
      shutDown();
    }
  }
